...
def temporal_padding(x, paddings=(1, 0), pad_value=0, device='cpu'):
    ...
    output = T.zeros(x.size(0), x.size(1) + sum(paddings), x.size(2)).to(device)
...
def lookup_neighbors(atoms, edges, maskvalue=0, include_self=False, device='cpu'):
    ...
    masked_atoms = temporal_padding(atoms, (1, 0), pad_value=maskvalue, device=device)
