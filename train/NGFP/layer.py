...
class GraphConv(nn.Module):
    ...
    '''
    def __init__(self, input_dim, conv_width, max_degree=6, device='cpu'):
    ...
    def forward(self, *input, mask=None):
        ...
        neighbor_atom_features = lookup_neighbors(atoms, edges, include_self=True, device=self.device)
...
class GraphPool(nn.Module):
    ...
    def forward(self, atoms, edges):
        ...
        neighbor_atom_features = lookup_neighbors(atoms, edges, maskvalue=-np.inf, include_self=True, device=self.device)
        # For each atom, look up the featues of it's neighbour
